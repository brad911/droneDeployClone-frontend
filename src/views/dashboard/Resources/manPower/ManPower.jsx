...
const ManPower = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.get('/manPower', { params });
      setData(res.data.data.results || res.data);
      setTotal(res.data.data.totalResults || res.data.data.total || 0);
    } catch (error) {
      console.error(error);
      enqueueSnackbar('Failed to load man power data', { variant: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/manPower/${selected._id}`);
                ...
                {data.length > 0 ? (
                  data.map((row) => (
                    <TableRow key={row._id} hover>
                      <TableCell>
                        {row.createdAt
                          ? new Date(row.createdAt).toLocaleDateString('en-GB') // dd/mm/yyyy
                          : '-'}
                      </TableCell>
